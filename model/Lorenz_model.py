...
from .base import *
from .utils.utils import PositionalEncodingLayer, View
'''
================================
Features for Lorenz96 system
================================
'''

class L96_phi_O(phi_O_BASE):
    def __init__(self, config, *args, **kwargs) -> None:
        ...
        for i in range(len(self.hidden_dims)):
            if i == 0:
                ...
                features.append(nn.Tanh())
            else:
                features.append(nn.Linear(self.hidden_dims[i-1], self.hidden_dims[i]))
                features.append(nn.Tanh())
        features = nn.Sequential(*features)
        super(L96_phi_O, self).__init__(features, *args, **kwargs)
    


class L96_hist_1dcov_features(nn.Module):
    def __init__(self, 
                 input_dim:int, 
                 history_len:int, 
                 hidden_dims:list, 
        ...
        super(L96_hist_1dcov_features, self).__init__(*args, **kwargs)
...
class L96_phi_H(phi_H_BASE):
    def __init__(self, config, *args, **kwargs) -> None:
        ...
        features = L96_hist_1dcov_features(input_dim=self.input_dim, 
        ...
        super(L96_phi_H, self).__init__(features, hist_w=self.history_len, *args, **kwargs)


class L96_phi_S(phi_S_BASE):
    def __init__(self, config, *args, **kwargs) -> None:
        ...
        for i in range(len(self.hidden_dims)):
            if i == 0:
                ...
                if i < len(self.hidden_dims) - 1:
                    features.append(nn.Tanh())
        features = nn.Sequential(*features)
        super(L96_phi_S, self).__init__(features, *args, **kwargs)

class L96_phi_inv_S(phi_inv_S_BASE):
    def __init__(self, config, *args, **kwargs) -> None:
        self.input_dim = config.state_dim
        self.hidden_dims = config.state_feature_dim
        features = nn.ModuleList()
        ...
        features = nn.Sequential(*features)
        super(L96_phi_inv_S, self).__init__(features, *args, **kwargs)
    
'''
=============================
models for Lorenz96 system
=============================
'''

class L96_foward_model(forward_model):
    def __init__(self, config, *args, **kwargs) -> None:
        phi_S = L96_phi_S(config)
        phi_inv_S = L96_phi_inv_S(config)
        seq_length = config.seq_length
        super(L96_foward_model, self).__init__(phi_S=phi_S,
                                               phi_inv_S=phi_inv_S, 
                                               seq_length=seq_length,
                                               *args, **kwargs)
        
class L96_inv_obs_model(inverse_model):
    def __init__(self, config, *args, **kwargs) -> None:
        phi_O = L96_phi_O(config)
        phi_H = L96_phi_H(config)
        phi_S = L96_phi_S(config)
        phi_inv_S = L96_phi_inv_S(config)
        rate = config.rate
        super(L96_inv_obs_model, self).__init__(phi_O=phi_O, 
                                                phi_H=phi_H, 
                                                phi_S=phi_S, 
                                                phi_inv_S=phi_inv_S, 
                                                rate=rate,
